...
namespace Yarn
{
    ...
    internal class VirtualMachine
    {
        ...
        private bool onlineTakeOver = false;
        ...
        public void SetSelectOptionOnline(int selectedOptionID)
        {
            // We won't be traveling to next node ourselves, so lets just clear the options
        ...
        internal void Continue()
        {
            ...
            // Prevent VM from running instructions by itself, since it will be controlled by remote client 
            // using SetProgramCount() method
            if (onlineTakeOver) return;
            ...
            while (CurrentExecutionState == ExecutionState.Running)
            {
                ...
                dialogue.OnProgramCountChange?.Invoke(state.programCounter);
        ...
        {
            state.programCounter = value - 1;
            Instruction currentInstruction = currentNode.Instructions[state.programCounter];
            RunInstruction(currentInstruction);

            state.programCounter++; 
            // Don't call dialogue.OnProgramCountChange?.Invoke(state.programCounter); here
            // cause we have manually changed programCount and are aware what the value of it now
            if (state.programCounter >= currentNode.Instructions.Count)
            {
                NodeCompleteHandler?.Invoke(currentNode.Name);
                CurrentExecutionState = ExecutionState.Stopped;
        ...
        public void SetOnlineTakeOver(bool value)
        {
            onlineTakeOver = value;
        }

        internal void RunInstruction(Instruction i)
        {
            dialogue.OnOperationCall?.Invoke(i.Opcode);
